...
uint8_t main(){
  ...
  while(1){
	...
	if(switch_state_servo != old_switch_state_servo){//if the switch has been flipped
		if(switch_state_servo == 0){	//set servo position appropriately
			OCR1A = 300;
		}else{
			OCR1A = 700;
		}
		old_switch_state_servo = switch_state_servo;
	}
  }
}
